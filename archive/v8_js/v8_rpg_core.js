//=============================================================================
// rpg_core.js - community-3.0 (pixi v8)
// https://github.com/RyanBram/RPG-MV-Ace
//=============================================================================
...
Bitmap.prototype._createBaseTexture = function (source) {
  var SourceClass =
    source instanceof HTMLImageElement ? PIXI.ImageSource : PIXI.CanvasSource;
  this.__baseTexture = new SourceClass({
    autoGenerateMipmaps: false,
    resource: source,
  });
  if (!this.texture) {
    this.texture = new PIXI.Texture({ source: this.__baseTexture });

    if (this._smooth) {
      this._baseTexture.scaleMode = "linear";
    } else {
...
Graphics.render = function (stage) {
  if (!this._renderer) {
    // Renderer not ready yet (async initialization)
    return;
  }
...
Graphics.isWebGL = function () {
  // For compatibility, return true for both WebGL and WebGPU (both use ShaderTilemap)
  return (
    this._renderer && (this._renderer.type === 1 || this._renderer.type === 2)
  ); // Pixi v8: 1 = WebGL, 2 = WebGPU
};

/**
 * Checks whether the renderer type is WebGPU.
 *
 * @static
 * @method isWebGPU
 * @return {Boolean} True if the renderer type is WebGPU
 */
Graphics.isWebGPU = function () {
  return this._renderer && this._renderer.type === 2; // Pixi v8: 2 = WebGPU
};
...
/**
 * Checks whether the current browser supports WebGPU.
 *
 * @static
 * @method hasWebGPU
 * @return {Boolean} True if the current browser supports WebGPU.
 */
Graphics.hasWebGPU = function () {
  try {
 ...
 * @private
 */
Graphics._createRenderer = async function () {
  var rendererOptions = {
    width: this._width,
    height: this._height,
    view: this._canvas,
  ...
  // Honor the preferred renderer type (set by SceneManager.preferableRendererType).
  // Pixi's autoDetectRenderer reads `options.preference` to prefer a renderer.
  if (this._rendererType && this._rendererType !== "auto") {
    rendererOptions.preference = this._rendererType;
  }

  try {
    this._renderer = await PIXI.autoDetectRenderer(rendererOptions);
...
Graphics._updateModeBox = function () {
  if (this._modeBox) {
    var text = document.getElementById("modeText");
    if (text) {
      // Check WebGPU FIRST, because isWebGL() returns true for both WebGL and WebGPU
      if (this.isWebGPU()) {
        text.innerHTML = "WebGPU";
      } else if (this.isWebGL()) {
        text.innerHTML = "WebGL";
      } else {
        text.innerHTML = "Canvas";
      }
    }
  }
};
...
Sprite.voidFilter = new PIXI.AlphaFilter();

Sprite.prototype.initialize = function (bitmap) {
  var texture = new PIXI.Texture(new PIXI.TextureSource());

  var baseInstance = new PIXI.Sprite(texture);

  // Store original PIXI blendMode descriptor before Object.assign
  // This is needed because we'll override blendMode but need to call PIXI's setter
  if (!Sprite._pixiBlendModeDescriptor) {
    // Get blend mode descriptor from the base instance (after Object.assign copies it)
    Sprite._pixiBlendModeDescriptor = Object.getOwnPropertyDescriptor(
      baseInstance,
      "blendMode"
    );
    // If not on instance, try Container prototype (PIXI v8 defines it there)
    if (!Sprite._pixiBlendModeDescriptor) {
      var proto = Object.getPrototypeOf(baseInstance);
      while (proto && !Sprite._pixiBlendModeDescriptor) {
        Sprite._pixiBlendModeDescriptor = Object.getOwnPropertyDescriptor(
          proto,
          "blendMode"
        );
  ...
  // [V8 Fix] Setup onRender callback for Pixi v8
  // This ensures transforms are applied during render cycle
  var self = this;
  this.onRender = function () {
    // Sprite doesn't need complex _onRenderUpdate like Tilemap
    // But having onRender ensures Pixi v8 updates transforms properly
  };

  // [V8 Fix] Initialize anchor, scale, and pivot to trigger transform updates
...
Object.defineProperty(Sprite.prototype, "bitmap", {
  ...
  set: function (value) {
    if (this._bitmap !== value) {
      ...
      if (value) {
        this.texture = new PIXI.Texture({ source: value.baseTexture }); // [V8 Fix] Use correct Texture constructor with source parameter
...
Object.defineProperty(Sprite.prototype, "blendMode", {
  ...
  set: function (value) {
    ...
    // Convert numeric blend mode (0-3) to Pixi v8 string format
    var pixiBlendMode;
    switch (value) {
      case 0:
        pixiBlendMode = "normal";
        break;
      case 1:
        pixiBlendMode = "add";
        break;
      case 2:
        pixiBlendMode = "multiply";
        break;
      case 3:
        pixiBlendMode = "screen";
        break;
      default:
        pixiBlendMode = "normal";
        break;
    }
    // Use stored PIXI blendMode descriptor to call original setter
    if (
      Sprite._pixiBlendModeDescriptor &&
      Sprite._pixiBlendModeDescriptor.set
    ) {
      Sprite._pixiBlendModeDescriptor.set.call(this, pixiBlendMode);
    }
  },
  configurable: true,
});

// [V8 Fix] Override x and y setters to trigger PIXI v8's transform update system
// Root cause: RPG Maker sets sprite.x and sprite.y directly,
// but PIXI v8 expects sprite.position.x/y which triggers ObservablePoint._onUpdate()
// Solution: Manually call _onUpdate() when x or y changes via direct assignment
/**
...
Object.defineProperty(Sprite.prototype, "x", {
  ...
  set: function (value) {
    if (this.position.x !== value) {
      ...
      // Manually trigger transform update for PIXI v8
      if (this._onUpdate) {
        this._onUpdate();
      }
    }
...
/**
 * Initialize anchor to trigger transform updates when anchor changes.
 * [V8 Fix] In PIXI v8, anchor is an ObservablePoint that needs explicit callbacks
 * to notify the transform system when values change.
 *
 * @method _initializeAnchor
 * @private
 */
Sprite.prototype._initializeAnchor = function () {
  var self = this;

  // Get the PIXI anchor ObservablePoint (already exists from baseInstance)
  if (this.anchor && !this._anchorInitialized) {
    // Store original _onUpdate callback if it exists
    var originalOnUpdate = this.anchor._onUpdate;

    // Wrap the _onUpdate to also trigger sprite's transform update
    this.anchor._onUpdate = function (x) {
      // Call original PIXI callback first
      if (originalOnUpdate) {
        originalOnUpdate.call(this, x);
      }
      // Manually trigger transform update for PIXI v8
      // Force transform recalculation
      if (self.transform) {
        self.transform.updateLocalTransform();
      }
    };
...
/**
 * Initialize scale to trigger transform updates when scale changes.
 * [V8 Fix] In PIXI v8, scale is an ObservablePoint that needs explicit callbacks
 * to notify the transform system when values change.
 *
 * @method _initializeScale
 * @private
 */
Sprite.prototype._initializeScale = function () {
  var self = this;

  // Get the PIXI scale ObservablePoint (already exists from baseInstance)
  if (this.scale && !this._scaleInitialized) {
    // Store original _onUpdate callback if it exists
    var originalOnUpdate = this.scale._onUpdate;

    // Wrap the _onUpdate to also trigger sprite's transform update
    this.scale._onUpdate = function (x) {
      // Call original PIXI callback first
      if (originalOnUpdate) {
        originalOnUpdate.call(this, x);
      }
...
/**
 * Initialize pivot to trigger transform updates when pivot changes.
 * [V8 Fix] In PIXI v8, pivot is an ObservablePoint that needs explicit callbacks
 * to notify the transform system when values change.
 *
 * @method _initializePivot
 * @private
 */
Sprite.prototype._initializePivot = function () {
  var self = this;

  // Get the PIXI pivot ObservablePoint (already exists from baseInstance)
  if (this.pivot && !this._pivotInitialized) {
    // Store original _onUpdate callback if it exists
    var originalOnUpdate = this.pivot._onUpdate;

    // Wrap the _onUpdate to also trigger sprite's transform update
    this.pivot._onUpdate = function (x) {
      // Call original PIXI callback first
      if (originalOnUpdate) {
        originalOnUpdate.call(this, x);
      }
...
Sprite.prototype.update = function () {
  ...
  // [V8 Fix] Text. Force bitmap texture update untuk memastikan visual refresh
  if (this._bitmap) {
...
Sprite.prototype._refresh = function () {
  ...
  if (realW > 0 && realH > 0) {
    if (this._needsTint()) {
      ...
      this.texture.source = this._tintTexture;
      this.texture.frame.copyFrom(new Rectangle(0, 0, realW, realH));
    } else {
      if (this._bitmap) {
        this.texture.source = this._bitmap.baseTexture;
      }
      this.texture.frame.copyFrom(this._realFrame);
      this.texture.updateUvs();
    }
  } else if (this._bitmap) {
    this.texture.frame.copyFrom(Rectangle.emptyRectangle);
    this.texture.updateUvs();
  } else {
    this.texture.source.width = Math.max(
      this.texture.source.width,
      this._frame.x + this._frame.width
    );
    this.texture.source.height = Math.max(
      this.texture.source.height,
      this._frame.y + this._frame.height
    );
    this.texture.frame.copyFrom(this._frame);
    this.texture.updateUvs();
  }
  this.texture._updateID++;

  // [PIXI v8 FIX] Notify sprite bahwa texture berubah
  // PIXI v8 migration guide: "sprites no longer get notified if a texture's UVs have been modified"
  // "sprite.onViewUpdate() guarantees the texture changes will be reflected on the sprite"
  // https://pixijs.com/8.x/guides/migrations/v8#other-breaking-changes
...
Sprite.prototype._createTinter = function (w, h) {
  ...
  if (!this._tintTexture) {
    this._tintTexture = new PIXI.CanvasSource({ resource: this._canvas });     // [V8 Fix] Use CanvasSource instead of TextureSource for canvas elements
  }
...
Tilemap.prototype.initialize = function () {
  ...
  // [V8 Fix] Setup onRender callback instead of updateTransform
  var self = this;
  this.onRender = function () {
    self._onRenderUpdate();
  };
...
/**
 * @method _onRenderUpdate
 * @private
 */
Tilemap.prototype._onRenderUpdate = function () {
...
function ShaderTilemap() {
  ...
  // [V8 Fix] Override onRender callback for ShaderTilemap
  var self = this;
  this.onRender = function () {
    self._onRenderUpdate();
  };
...
ShaderTilemap.prototype._hackRenderer = function (renderer) {
  ...
  if (af == 3) af = 1;
  renderer.renderPipes.tilemap.tileAnim[0] = af * this._tileWidth;
  renderer.renderPipes.tilemap.tileAnim[1] =
...
ShaderTilemap.prototype.refreshTileset = function () {
  var bitmaps = this.bitmaps.map(function (x) {
    return x._baseTexture ? new PIXI.Texture({ source: x._baseTexture }) : x; // [V8 Fix] Use correct Texture constructor with source parameter
  });
 ...
 * @private
 */
ShaderTilemap.prototype._onRenderUpdate = function () {
  ...
  this._sortChildren();
  // [V8 Fix] Custom Workaround - Force mark children sebagai dirty. Untuk Sprite di MAP
  for (var i = 0; i < this.children.length; i++) {
    var child = this.children[i];
    // Mark position sebagai dirty
    if (child._didChangeId !== undefined) {
      child._didChangeId++;
    }
    // Force update bounds untuk trigger recalculation
    if (child._bounds) {
      child._bounds.updateID++;
    }
  }
};
...
ShaderTilemap.prototype._createLayers = function () {
  ...
  if (!this.lowerZLayer) {
    ...
    this.lowerLayer = new PIXI.tilemap.CompositeTilemap(0, [], useSquareShader);
    this.lowerLayer.shadowColor = new Float32Array([0.0, 0.0, 0.0, 0.5]);
    this.lowerLayer.zIndex = 0;
    this.lowerZLayer = this.lowerLayer; // Alias untuk sisa kode

    this.upperLayer = new PIXI.tilemap.CompositeTilemap(4, [], useSquareShader);
...
TilingSprite.prototype.initialize = function (bitmap) {
  ...
  // [V8 Fix] Setup observer for origin Point changes to trigger PIXI v8 transform update
  // This is needed for parallax scrolling to work properly
  // IMPORTANT: We must modify the existing origin object from PIXI, not create a new one
  var self = this;

  // Check if origin already exists from PIXI.TilingSprite
  if (!this.origin) {
    /**
  ...
  // Now setup observers on the existing origin object
  var originPoint = this.origin;
  var _x = originPoint.x || 0;
  var _y = originPoint.y || 0;

  Object.defineProperty(originPoint, "x", {
    get: function () {
      return _x;
    },
    set: function (value) {
      if (_x !== value) {
        _x = value;
        // Directly update tilePosition for immediate effect
        self.tilePosition.x = Math.round(-_x);
        // Manually trigger view update for PIXI v8
        if (self.onViewUpdate) {
          self.onViewUpdate();
        }
      }
    },
  ...
  Object.defineProperty(originPoint, "y", {
    get: function () {
      return _y;
    },
    set: function (value) {
      if (_y !== value) {
        _y = value;
        // Directly update tilePosition for immediate effect
        self.tilePosition.y = Math.round(-_y);
        // Manually trigger view update for PIXI v8
        if (self.onViewUpdate) {
          self.onViewUpdate();
        }
 ...
 * @private
 */
TilingSprite.prototype._onRenderUpdate = function () {
...
TilingSprite.prototype._onBitmapLoad = function () {
  this.texture = new PIXI.Texture({ source: this._bitmap.baseTexture }); // [V8 Fix] Use correct Texture constructor with source parameter
  this._refresh();
};
...
ScreenSprite.prototype.setColor = function (r, g, b) {
  if (this._red !== r || this._green !== g || this._blue !== b) {
    ...
    //rect first before fill
    graphics.rect(-Graphics.width * 5, -Graphics.height * 5, Graphics.width * 10, Graphics.height * 10);
    graphics.fill(intColor);
  }
};
...
// [V8 Fix] Override x and y setters for Window to trigger PIXI v8's transform update
// Same fix as Sprite - Window also sets x/y directly, bypassing ObservablePoint
/**
 ...
 * @private
 */
Window.prototype._onRenderUpdate = function () {
  this._updateCursor();
  this._updateArrows();
...
WindowLayer.prototype.initialize = function () {
  ...
  this._windowMask.fill(0xffffff, 1);
  this._windowMask.rect(0, 0, 0, 0);
  this._windowMask.endFill();
  this._windowRect = new PIXI.Rectangle(0, 0, 0, 0);
...
WindowLayer.voidFilter = new PIXI.AlphaFilter();
...
// [V8 Fix] Override x and y setters for WindowLayer to trigger PIXI v8's transform update
// Same issue as Sprite and Window - WindowLayer.move() sets x/y directly,
// but PIXI v8 expects position.x/y which triggers ObservablePoint._onUpdate()
// This fix ensures WindowLayer position updates properly when aspect ratio changes
/**
 ...
 * @type Number
 */
Object.defineProperty(WindowLayer.prototype, "x", {
  get: function () {
    return this.position.x;
  },
  set: function (value) {
 ...
 * @type Number
 */
Object.defineProperty(WindowLayer.prototype, "y", {
  get: function () {
    return this.position.y;
  },
  set: function (value) {
...
WindowLayer.prototype._maskWindow = function (window, shift) {
  ...
  this._windowMask.fill(0xffffff);
  this._windowMask.rect(
 ...
 */
function ToneFilter() {
  var baseInstance = new PIXI.ColorMatrixFilter();
  Object.assign(this, baseInstance);
}

ToneFilter.prototype = Object.create(PIXI.ColorMatrixFilter.prototype);

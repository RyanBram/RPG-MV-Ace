//=============================================================================
// rpg_managers.js - community-3.0 (pixi v8)
...
StorageManager.localFileDirectoryPath = function () {
  ...
  if (Utils.isDesktop()) {
    ...
    // For Electron in production, save files are in a different location
    if (Utils.isElectron() && !Utils.isOptionValid("test")) {
...
SceneManager.preferableRendererType = function () {
  if (Utils.isOptionValid("canvas")) {
    ...
    // Default: auto - let PixiJS decide based on its priority
    return "auto";
  }
};
...
SceneManager.checkWebGPU = function () {
  if (!Graphics.hasWebGPU()) {
    throw new Error(this.getWebGPUErrorMessage());
  }
};

SceneManager.getWebGPUErrorMessage = function () {
...
SceneManager.catchException = function (e) {
  if (e instanceof Error) {
    // Check if this is a WebGPU-related error
    var isWebGPUError = this._isWebGPUError(e);

    if (isWebGPUError) {
      Graphics.printError("WebGPU Error", this.getWebGPUErrorMessage());
    } else {
...
SceneManager._isWebGPUError = function (error) {
  // Check error message for WebGPU-related keywords
  var message = error.message || "";
  var stack = error.stack || "";
  var combined = (message + " " + stack).toLowerCase();
  ...
  return webgpuKeywords.some(function (keyword) {
    return combined.indexOf(keyword) !== -1;
  });
};

...
(function () {
      ...
      self.addEventListener("message", function (event) {
        initializeP.then(function (funcs) {
          ...
          if (event.data.eof) {
            ...
            if (len) {
              console.warn(
                "not all the input data was decoded. remaining: " +
                  len +
                  "[bytes]",
              );
            }
  ...
  var initializeWorkerP = new Promise(function (resolve, reject) {
    ...
    httpGet(directoryPath + "stbvorbis_stream_asm.js")
        ...
        reject(
          new Error(
            "asmjs version is not available (HTTP status: " +
              err.status +
              " on stbvorbis_stream_asm.js). Deploy stbvorbis_stream_asm.js at the same place as stbvorbis.js.",
          ),
        );
      });
  });
  ...
  stbvorbis.decodeStream = function (outCallback) {
    ...
    initializeWorkerP.then(function (worker) {
      ...
      var onmessage = function (event) {
        ...
        if (result.eof) {
          outCallback({ data: null, sampleRate: 0, eof: true, error: null });
          worker.removeEventListener("message", onmessage);
          return;
        }
        outCallback({
      ...
      inCallbackImpl = function (input) {
        if (input.eof) {
          worker.postMessage({ id: currentId, buf: null, eof: true });
          return;
        }
        var buf = input.data;
        worker.postMessage({ id: currentId, buf: buf, eof: false }, [
